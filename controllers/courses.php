...
<?php
	if(isset($_POST['unenroll'])){
		...
		while($row=mysqli_fetch_array($res12)){
			$eid=(int)$row['id'];
			$query2="DELETE FROM exam_marks WHERE enrollmentId='{$eid}'";
			$res112=mysqli_query($connection, $query2);

			$query3="DELETE FROM class_posts WHERE classId='{$cid}' AND userId='{$sid}'";
			$res112=mysqli_query($connection, $query3);
		}
